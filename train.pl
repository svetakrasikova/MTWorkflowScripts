...
our ($engine, $corpus, $source, $target, $lm_path, $bin_dir, $base_path, $first_step, $last_step, $log_dir, $seg_script, $segmenter, $seg_path, $seg_model, $reorder_cmd, $build_lm, $build_recaser, $force, $device, $binarise, $fiscal_year, $archive, $perProduct);

unless (defined($corpus) && ((defined($source) && defined($target)) || defined($engine)) && defined($lm_path) && defined($bin_dir) && defined($fiscal_year) && (!(((defined $target && defined $source) || (defined $engine && $engine =~ m!_xx/?!i)) && $target eq "en" && $source ne "xx") || defined($archive))) {
...
if (!$forked || !$pid) {
	...
	if (defined $build_recaser && (defined $force || !(-e "$build_recaser/moses.ini"))) {
		...
		print STDERR "\n";
		system("$base_path/train-recaser.perl --dir $build_recaser --corpus $build_recaser/corpus.tok.$target.bz2 --train-script \"/usr/local/bin/moses-train-model.perl\" --language $target --device $device --binarise $binarise >>$logFile") == 0
...
if ((!$sub_pid || !$sub_fork) && (defined $build_lm && (defined $force || !(-e "$lm_path/lm5bin")))) {
	...
	system(($build_lm ne "1" ? "$build_lm/" : "")."lmplz -o 5 -T /tmp <$corpusDir/corpus.tok.$target.bz2> $lm_path/lm5arpa && build_binary -q 8 -a 255 -T /tmp -S 33% trie $lm_path/lm5arpa $lm_path/lm5bin && rm -vf $lm_path/lm5arpa") == 0
...
unless (!(defined $force) && -e "./model/moses.ini" || $first_step > $last_step) {
	...
	my $command = "/usr/local/bin/moses-train-model.perl -parallel -phrase-translation-table phrase-table:12 -first-step $first_step -last-step $last_step -external-bin-dir \"$bin_dir\" -temp-dir /tmp -verbose -continue -root-dir . -f $source -e $target -corpus-dir \"$corpusDir\" -corpus \"$corpusclean\" -alignment grow-diag-final-and -reordering msd-bidirectional-fe -lm 0:5:$lm_path/lm5bin:8 -device $device -binarise $binarise -max-lexical-reordering 6";
	...
	system "/usr/bin/perl -i.bak -pe 's!^(.*)\\..*?\\-fe\\.gz\$!\$1!' ./model/moses.ini";
	system "/usr/bin/perl -i.ventzi -pe 's,Volumes/.*/Autodesk,local/cms,' ./model/moses.ini";
}
...
system("tar -cvjf $archiveFile corpus ".($first_step > $last_step ? "" : "giza.* model/aligned.grow-diag-final-and* model/lex.* model/phrase-table.gz ").(defined $build_recaser ? "recaser/phrase-table.gz " : "")."&& rm -rf corpus ".($first_step > $last_step ? "" : "giza.* model/aligned.grow-diag-final-and* model/lex.* model/phrase-table.gz ").(defined $build_recaser && ($source eq "xx" || $target ne "en") ? "recaser/phrase-table.gz recaser/corpus.tok.$target.bz2" : "")) == 0
