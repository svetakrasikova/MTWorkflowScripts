...
my (@languages, %localeMap, $fy, %engines, %ports, %servers, %segmenters);
our $serverSetupFile;
if ($serverSetupFile) {
...
sub updateSetupData {
	...
	@languages = (); %localeMap = (); $fy = 0; %engines = (); %ports = (); %servers = (); %segmenters = ();
...
$server_sock->sockopt(SO_SNDLOWAT, 512);
...
$SIG{CHLD} = 'IGNORE';
$SIG{PIPE} = sub {warn encode("utf-8", "!!! ERROR: Some connection was dropped!\n")};
...
#			print LOG encode("utf-8", "Conection lost/timeout!\n");
			last;
		}
...
#			if ($update) {
#				print $client_sock encode("utf-8", "Updating MT Info Service Setup Data from disk…".(&updateSetupData ? " done." : " no update needed!")."\nUpdating MT Info Service Glossary Data from disk…".(&updateGlossaryData ? " done." : " no update needed!")."\nPlease enter new command:\n");
#				next;
#			}
			...
			foreach $srcLang ($srcLang ? ($srcLang) : @languages) {
				...
				foreach $trgLang ($trgLang ? ($trgLang) : @languages) {
					...
					unless ($oper || $translate) {
						...
						if ($engine) {
							if ($engine eq "n/a") {
								print $client_sock encode("utf-8", "{engine => \"n/a\", translate => 1}\n") if $translate;
								print $client_sock encode("utf-8", "$srcLang => $trgLang engine not installed!\n");
							} else {
								$engine = $fy."_".(uc $srcLang)."-".(uc $trgLang)."_".$engine unless $engine =~ /_.*_/;
							}
							if ($oper) {
	...
	if ($finished) {
		...
		($hostport) = $hostport =~ /^(\d+)$/;
		for (;;) {
			exec "/bin/sleep 60; /local/cms/bin/mt_info_server.pl -hostport=$hostport -DEBUG 2>>/local/cms/nohup.out"
...
sub check {
	...
	my $serverCheck = sub {
		...
		if ($collectStats) {
			my $statistics = `ssh $server '/local/cms/bin/countMosesWords.pl -logDir=/local/cms/LOG -filterEngine=$engine'`;
...
sub start {
	...
	foreach my $server (@{$data->{server}}) {
		if ($liveServers{$server}) {
			...
			my $result = system("ssh -n $server \"cd /local/cms; nohup moses_server.ventzi.pl -engine=$engine -hostport=$data->{port} >>nohup.out 2>\&1 \&\"");
...
sub translate {
	my ($config, $engine, $input, $client_sock, $product, $getScore) = @_;
	...
	my %temp;
	do {
		%temp = map {$_ => 1} @{&check(1, $config, undef, $engine)};
		sleep(2) unless %temp;
	} while (!%temp);
...
#	$jobSize = min(scalar @$input, 5) if $jobSize < 5;
	print STDERR encode("utf-8", "$ID: "."Job size set to $jobSize; translating using $engine…\n") if $DEBUG;
	
	$tstamp = strftime("%Y.%m.%d_%H.%M.%S", localtime(time()));
	open my $output, ">/local/cms/LOG/dummy.$tstamp.$ID.txt";
	print $output encode "utf-8", "$_\n" foreach @$input;
	print $client_sock encode"utf-8", "$_\n" foreach @$input;
...
#			{ lock $lostClient;
#				if ($lostClient) {
#					print STDERR encode("utf-8", "$ID: "."Client lost while processing on server $host with engine $engine!\n") if $DEBUG;
#					$mustExit = 1;
#				}
#			}
#			return if $mustExit;
...
#			my $dataRef;
#			($job, $dataRef) = %$job;
#			print STDERR encode("utf-8", "$ID: "."Processing job $job on server $host…\n") if $DEBUG;
#			
#			{ lock $output;
...
#							
#							($mosesSocket) = $select->can_read(30);
#							unless ($mosesSocket) {
#								$mosesError = 1;
#								last;
#							}
...
#				$output->{$job}->[1] = 1;
#				
#				if ($lastDoneJob < (keys %$output) - 1 && defined $output->{$lastDoneJob + 1} && $output->{$lastDoneJob + 1}->[1]) {
#					my $select = IO::Select->new();
...
#							++$lastDoneJob;
#							print STDERR encode("utf-8", "$ID: "."--> Printing job $lastDoneJob…\n") if $DEBUG;
#							print $local_socket encode("utf-8", $_) foreach @{$output->{$lastDoneJob}->[0]};
#						}
#					}
#				}
#			}
...
#		}
#
#		print STDERR encode("utf-8", "$ID: "."Finished processing on server $host with engine $engine…\n") if $DEBUG;
#		return;
#	};
...
#		print STDERR encode("utf-8", "$ID: "."Creating MT threads…\n") if $DEBUG;
#		my @threads = map { scalar threads->create($translate, $_, $config->{port}) } shuffle keys %$liveServers;
#	
#		$_->join() foreach @threads;
#
#		if ($lostClient) {
#			print STDERR encode("utf-8", "$ID: "."ERROR: Client lost while translating with engine $engine!\n") if $DEBUG;
#			return;
#		}
#
#		print STDERR encode("utf-8", "$ID: "."--> ".(scalar keys %$liveServers)." servers left…\n") if $DEBUG;
...
sub matchGlossary {
	...
	return unless defined $glossary{$product} && defined $glossary{$product}->{languages}->{$targetLanguage};
