...
# Created by Ventsislav Zhechev on 12 Apr 2012
# Last modified by Ventsislav Zhechev on 12 Apr 2012
#
#
# Reorder English data to better match German word order
#
# ChangeLog
# v0.1
# Initial version based on reorder_jpn.pl
#
################################
...
# This subroutine takes care of the second pre-processing step and sends the segments to the parser. Too long/complex sentences are being pushed through unchanged by only allowing the parser to see an ‘’ character. The reason is that too long/complex sentences have little chance of being correctly parsed/reordered.
sub sendToParser {
	my $thruLine = shift;
	++$thruLineCounter;
	...
	{ lock(%cmds);
		if ((() = split ' ', $thruLine, -1) > 50 || $thruLine =~ /,/g > 10) {
			...
			$cmds{$thruLineCounter} = $1 if $thruLine =~ s/$re//x;
			...
			$thruLine ||= ""; #Mask lines that need to be skipped so that the parser doesn’t choke on them
...
#			$thruLine =~ s/-(L|R)(S|C)B-/my ($l, $s) = ($1 eq 'L', $2 eq 'S'); if ($l && $s) {'['} elsif ($s) {']'} elsif ($l) {'{'} else {'}'}/ge;
		...
		#REORDER
#		$thruLine = &reorder($thruLine, $pseudoThruID) if $thruLine =~ /^\(.*\)$/ && !($thruLine =~ / \){2,3}$/); #Do not reorder empty or skipped lines
		#DEBUG
		print STDOUT "$thruLine\t$pseudoThruID\n" if $thruLine =~ /^\(.*\)$/ && !($thruLine =~ / \){2,3}$/); #Do not reorder empty or skipped lines
		next;
		
		#POSTPROCESS ONE
		# cmd line
		{ lock(%cmds);
			if (defined $cmds{$pseudoThruID}) {
#				print STDERR "CHECKING LINE “$thruLine” FOR ODDITIES…\n";
				if (!$thruLine || $thruLine =~ / \){2,3}$/) {
					print STDERR "Bad line $sentID: “$thruLine” with cmd “$cmds{$pseudoThruID}”\n" if $debug;
					$thruLine = $cmds{$pseudoThruID};
				} else {
					$thruLine .= " $cmds{$pseudoThruID}";
				}
			}
		...
		# clean space (Not sure if it’s really necessary)
		$thruLine =~ s/(^\s+)|(\s+$)//g;
		$thruLine =~ s/\s+/ /g;
		...
		{ lock(%thrus);
			if (!defined($thrus{$pseudoThruID-1}) || $thrus{$pseudoThruID} != $thrus{$pseudoThruID-1}) { #This is not a continuation of a previous line
				...
				unless (defined $thrus{$pseudoThruID+1} && $thrus{$pseudoThruID} == $thrus{$pseudoThruID+1}) {
					undef $thrus{$pseudoThruID}; #Only undef if there is no continuation to this line
					$printLine = 1;
				}
				...
				if (!defined($thrus{$pseudoThruID+1}) || $thrus{$pseudoThruID} != $thrus{$pseudoThruID+1}) { #Cleanup if there is no continuation to this line
					for (my $id = $pseudoThruID; defined $thrus{$id}; ) {
						undef $thrus{$id} if defined $thrus{--$id} && $thrus{$pseudoThruID} == $thrus{$id};
					}
					undef $thrus{$pseudoThruID};
					$printLine = 1;
				}
			}
		}
